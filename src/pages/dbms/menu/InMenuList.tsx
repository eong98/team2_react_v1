...
import type { MenuType } from './Menu.ts';
...
export default function InMenuListView() {
  ...
  const [childMap, setChildMap] = useState<Record<number, MenuType[]>>({});
  const [loading, setLoading] = useState(true);
  const [reorderingKey, setReorderingKey] = useState<string | null>(null); // "top" | `child:${fkno}`
  const [deleteTarget, setDeleteTarget] = useState<MenuType | null>(null);
  const [collapsed, setCollapsed] = useState<Set<number>>(new Set());
  const [page, setPage] = useState(1);

  // 드래그 중에도 항상 최신 상태를 참조하기 위한 ref
  const topListRef = useRef<MenuType[]>([]);
  const childMapRef = useRef<Record<number, MenuType[]>>({});
  ...
  const askDelete = (item: MenuType) => setDeleteTarget(item);
   ...
   */
  const persistOrder = async (list: MenuType[]): Promise<MenuType[]> => {
